...
describe('http2SessionTimeout', () => {
  ...
  test('http2 close with async-await', async (t) => {
  ...
  test('http2s close with async-await', async (t) => {
    const server = createServer({
      http2: true,
      https: {
    ...
    await server.listen()
    const session = connect(server.listeningOrigin, { rejectUnauthorized: false })
    session.on('error', () => { })
    await once(session, 'connect')
    await server.close()
  })
