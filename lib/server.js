...
function normalizeServerOptions(options) {
  ...
  else serverOptions.http2SessionTimeout = 72000;

  if (typeof options.forceCloseConnections === "boolean")
    serverOptions.forceCloseConnections = options.forceCloseConnections;
  else serverOptions.forceCloseConnections = false;
...
function createHTTPSServer(options, requestListener) {
  ...
  const server = https.createServer(options.https, requestListener);
