...
function normalizeServerOptions(options) {
  ...
  else serverOptions.keepAliveTimeout = 72000;
  ...
  else serverOptions.connectionTimeout = 0;
  ...
  else serverOptions.maxRequestsPerSocket = 0;
  ...
  else serverOptions.requestTimeout = 0;
...
function selectServer(options, requestListener) {
  ...
  if (isCustom(serverOptions)) {
    return createCustomServer(serverOptions, requestListener);
  }

  if (isHTTP2(serverOptions)) {
    return createHTTP2Server(serverOptions, requestListener);
  }

  if (isHTTPS(serverOptions)) {
    return createHTTPSServer(serverOptions, requestListener);
  }

  return createHTTPServer(serverOptions, requestListener);
}
